...
app.put("/users/:id", async (c) => {
  ...
  const body = await c.req.json().catch(() => ({}));
  const user = await prisma.user.update({ where: { id }, data: { email: body.email, name: body.name } }).catch(() => null);
...
app.put("/units/:id", async (c) => {
  const id = c.req.param("id");
  const body = await c.req.json().catch(() => ({}));
  const unit = await prisma.unit.update({ where: { id }, data: { name: body.name, description: body.description } }).catch(() => null);
